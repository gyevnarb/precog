...
class SerializedDataset(interface.ESPDataset, minibatched_dataset.MinibatchedDataset):
    input_keys = ['player_future', 'agent_futures', 'player_past', 'agent_pasts', 'player_yaw', 'agent_yaws',
                  "overhead_features"]
    ...
    def get_minibatch(self, is_training, split='train', mb_idx=None, input_singleton=None,
        ...
        if self.Other_count > 0:
            ...
            other_pasts = np.stack([npu.fill_axis_to_size(
                np.asarray(k(_, 'agent_pasts'))[:self.Other_count], axis=0, size=self.Other_count_past) for _ in data],
                1)
        ...
        # Create phi.
        if input_singleton:
            feed_dict = input_singleton.to_feed_dict(
            ...
            if not return_metadata:
                ...
                return feed_dict, metadata_list.to_dict()
        else:
...
def get_sdt(bevs, filename_keys, sdt_clip_thresh, stamp=True, sdt_zero_h=0, sdt_zero_w=0., sdt_params={},
            sdt_params_name=''):
    """
...
def _create_sdt(bevs, sdt_filename, b, H, W, C, sdt_clip_thresh, stamp=True, sdt_zero_h=0, sdt_zero_w=0., sdt_params={},
                save=True):
