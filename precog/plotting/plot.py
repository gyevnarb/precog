...
def plot_figure(key, fig, partial_write_np_image_to_tb=None):
    ...
    if partial_write_np_image_to_tb is not None:
        return partial_write_np_image_to_tb(arr)
    else:
        return tf.contrib.summary.image(key, arr)
...
def plot_single_sampled_output(sampled_output, batch_index=0, fig=None, ax=None, render=True):
    ...
    # Plot future.
    fig, ax = plot_joint_trajectory(S[batch_index], key='sampled_trajectories', render=False, marker='o', zorder=1,
                                    alpha=.4, fig=fig, ax=ax, limit=limit)
    return fig, ax


def plot_whiskers(rollout, batch_index=0, k_slice=slice(0, 1), a_slice=slice(0, 5), period=5, fig=None, ax=None,
                  render=True):
...
def load_nuscenes(version='v1.0-mini', dataroot='/home/nrhinehart/data/datasets/nuscenes_raw/', verbose=True,
                  nusc=[None]):
...
def plot_past(S_past, b, fig, ax, limit, alpha=0.5):
    # Plot past.
    plot_joint_trajectory(S_past[b][None], key='sampled_trajectories', render=False, fig=fig, ax=ax, marker='d',
                          zorder=2, alpha=alpha, limit=limit)
    # Plot origin.
...
def plot_sample(sampled_output, expert=None, b=0, figsize=(4, 4), partial_write_np_image_to_tb=None, bev_kwargs={}):
    ...
    if expert is not None:
        plot_expert(expert, batch_index=b, fig=fig, ax=ax, render=False,
                    limit=tensoru.size(sampled_output.phi.overhead_features, 1), **expert_kwargs)
...
def plot_coordinate_frames(coordinate_frame_list, b=0, fig=None, axes=None, figsize=(8, 20)):
    ...
    bases_grid = np.concatenate(
        [_.apply(points_local, points_ein='bkaj')[b].numpy().reshape((-1, 2)) for _ in coordinate_frame_list], 0)
