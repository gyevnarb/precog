...
class ESPObjective:
    ...
    @abstractmethod
    def call(self, model_distribution, model_distribution_samples, target_distribution_minibatch, data_epsilon,
...
class ESPPhi(NumpyableTensorGroup):
    ...
    def tensor_init(self, S_past_world_frame, yaws, overhead_features, agent_presence, light_strings, is_training):
        pass
    ...
    def update_frames(self, S_t_car_frames, S_tm1_car_frames):
        ...
        velocity_in_world_frame = self.original_local2world.apply(velocity_in_car_frame, self.batch_str + 'aj',
                                                                  translate=False)
        ...
        nontrivial_motion_mask_tile = tf.tile(nontrivial_motion_mask[..., None, None],
                                              (1,) * (len(self.batch_str) + 1) + (2, 2))
        local_frame_R = tf.where(nontrivial_motion_mask_tile, local_frame_R_vel, local_frame_R_prev)
        self.current_local2world = similarityu.SimilarityTransform(R=local_frame_R, t=S_t_world_frame,
                                                                   scale=self.current_local2world.scale)
    ...
    def tensor_names(self):
        return (
        ['S_past_world_frame', 'yaws', 'overhead_features', 'agent_presence', 'S_past_car_frames', 'S_past_grid_frame',
...
def one_hotify_light_strings(light_strings):
    ...
    with tf.control_dependencies(
            [tf.compat.v1.assert_equal(none + green + yellow + intersection + red, tf.ones_like(none))]):
...
class ESPTrainingInput(NumpyableTensorGroupGroup):
    ...
    @classmethod
    def from_data(cls, S_past_world_frame, yaws, overhead_features, agent_presence, S_future_world_frame,
                  feature_pixels_per_meter,
                  light_strings,
                  is_training,
                  metadata_list=None,
        ...
        experts = ESPExperts(
            S_future_car_frames=S_future_car_frames, S_future_world_frame=S_future_world_frame,
            S_future_grid_frame=S_future_grid_frame)
    ...
    def to_singleton(self):
        ...
        S_future_world_frame = tf.compat.v1.placeholder(
            shape=self.experts.S_future_world_frame.shape, dtype=self.experts.S_future_world_frame.dtype,
        ...
        is_training = tf.compat.v1.placeholder(shape=self.phi.is_training.shape, dtype=self.phi.is_training.dtype,
                                               name="is_training")

        metadata_placeholders = [tf.compat.v1.placeholder(shape=_.array.shape, dtype=_.dtype, name=_.name) for _ in
                                 self.phi_m.metadata_list]
        ...
        ESPTrainingInput.sample_placeholders = ESPTrainingInput.placeholders[
                                               :expert_index] + ESPTrainingInput.placeholders[expert_index + 1:]
    ...
    def to_feed_dict(self, S_past_world_frame, yaws, overhead_features, agent_presence, S_future_world_frame,
                     metadata_list, light_strings, is_training):
        fd = tfutil.FeedDict(
            zip(self.placeholders,
                [S_past_world_frame, yaws, overhead_features, agent_presence, light_strings, S_future_world_frame,
                 is_training] + metadata_list.arrays))
        fd.validate()
        return fd
...
class ESPInference:
    def __init__(self, tensor_collections):
        ...
        self.training_input = Mock(
            ...
            {'phi': self.phi, 'phi_m': self.phi_metadata, 'experts': self.experts, 'experts_m': self.experts_metadata})
        ...
        def to_feed_dict(xself, S_past_world_frame, yaws, overhead_features, agent_presence, light_strings,
                         S_future_world_frame, metadata_list, is_training):
            return dict(zip(xself.placeholders,
                            [S_past_world_frame, yaws, overhead_features, agent_presence, light_strings,
                             S_future_world_frame, is_training]))
        ...
        self.phi.local2world, self.phi.world2local, self.phi.world2grid, self.phi.local2grid = ESPPhi.frames_init(
            self.phi.S_past_world_frame, self.phi.overhead_features, self.phi.yaws, yaws_in_degrees=True,
    ...
    def add(self, name, klass):
        ...
        for n in names:
            ...
            if t is None: log.warning(
                "ESPInference's member object is missing a member! '{}'. Class={}".format(n, klass))
            tensors_dict[n] = t
